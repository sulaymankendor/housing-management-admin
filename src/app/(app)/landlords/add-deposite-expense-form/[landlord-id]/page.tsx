...
import { depositeExpenseType } from "@/lib/types";
import { depositeExpenseSchema } from "@/lib/zodValidationSchema";
import { zodResolver } from "@hookform/resolvers/zod";
import { useParams } from "next/navigation";
import React, { useState } from "react";
import { Controller, useForm } from "react-hook-form";

function AddDepositeExpenseForm() {
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<depositeExpenseType>({
    resolver: zodResolver(depositeExpenseSchema),
    defaultValues: {
      tenantID: "",
      tenantName: "",
      depositedAmount: "",
  ...
  const onSubmit = async (data: depositeExpenseType) => {
    axisoRequest.post(
      "depositeExpenses",
      {
        ...data,
        //@ts-ignore
        depositedDate: formatDate(data.depositedDate),
    ...
    <div className="bg-white p-10 rounded-2xl border border-gray-200">
      <div>
        <h1 className="font-semibold text-lg text-black">
          Add an Deposite Expense
        </h1>
        <p className="text-gray-500 text-xs">
          Lorem ipsum dolor sit amet saul saul consectetur adipisicing elit.
          Praesent
        </p>
      </div>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex items-center gap-5 mx-auto mt-10">
          ...
          <div className="grid w-full">
            <Input
              placeholder="Expended Amount"
              id="totalAmount"
              {...register("expendedAmount")}
              type="number"
            />
            {errors.expendedAmount && (
              <p className="mt-1 text-xs text-red-600">
                {errors.expendedAmount.message}
              </p>
            )}
          </div>
        </div>
...
export default AddDepositeExpenseForm;
